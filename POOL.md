...
> **⚠ 号龄回归后（2026-07-16）：own vs buffer 同龄下无实质差异，别把 own 当"后备"囤。**
> ban 由号龄驱动（约 24-48h 寿命上限，见 `docs/STATUS.md` §4 / `AGENTS.md` 判死铁律 5），
> TTL 对所有 free/cli-chat-proxy 号一视同仁、不看域名来源。全量死亡率 buffer 45% ≪ own 86%
> **是号龄构成假象**（活号池 92% 是 buffer 偏年轻、own 只剩一小撮老号残留），非质量差。
> own 放货架上照样到龄死（实测 own >3d 仍 84% 死、活号中位 0.16d 放置就没再刷）。
> **结论：own 和 buffer 都是 24-48h 耗材，按"谁新鲜烧谁"统一消耗，"留后备"= 纯浪费。**
> 域名顶多二阶效应（自有域幸存率略高），一阶永远是号龄。复核：`python scripts/ban_regression.py`。
